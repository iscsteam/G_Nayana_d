...
BEST_MODEL_PATH = config['model']['best_model_path']  # Load from YAML
...
# New GradCAM class and functions
class GradCAM:
...
def apply_gradcam(model, image, transform):
    img_tensor = transform(image).unsqueeze(0).to(DEVICE)
    target_layer = model.efficientnet.features[-1]  # Last convolutional layer
    grad_cam = GradCAM(model, target_layer)
    heatmap = grad_cam.generate_cam(img_tensor)
    return heatmap
...
def infer(model, image_url, transform):
    ...
    heatmap = apply_gradcam(model, img, transform)
    gradcam_image = overlay_heatmap(img, heatmap)
    
    return predicted.item(), confidence.item(), gradcam_image
...
async def run_inference(patient_id: str, left_image: UploadFile = File(...), right_image: UploadFile = File(...)):
    ...
    left_class, left_confidence, left_gradcam = infer(model, left_image_url, transform)
    right_class, right_confidence, right_gradcam = infer(model, right_image_url, transform)
    
    # Generate detailed results
    left_result = generate_result(left_class, left_confidence)
    right_result = generate_result(right_class, right_confidence)
    
    # Save and upload GradCAM images
    left_gradcam_url = upload_image_to_azure(patient_id, io.BytesIO(cv2.imencode('.jpg', np.array(left_gradcam))[1]), 'left_gradcam.jpg')
    right_gradcam_url = upload_image_to_azure(patient_id, io.BytesIO(cv2.imencode('.jpg', np.array(right_gradcam))[1]), 'right_gradcam.jpg')
    
    # Prepare final results
    results = {
        "left_eye": {**left_result, "gradcam_url": left_gradcam_url},
        "right_eye": {**right_result, "gradcam_url": right_gradcam_url}
    }
